# Molan: Molan-API
#
# Author: Datyam Kumar <satyamkumar4@acm.org>
...
def build(id, source_code, input_data = None):
    ...
    main_file, source_file, parse_tree_file, input_file, output_file, err_file = extension("MIPS")
    ...
    try:
        ret_val = subprocess.run(["mipc", main_file], stdin = fin, stdout = fout, stderr = ferr, timeout=3, check=True, shell=False).returncode
    ...
    if delete(source_file, parse_tree_file, input_file, output_file, err_file) == -1:
