...
def build(id, source_code, input_data = None):
    ...
    ret_val =  subprocess.call(["g++", source_file, "-o", binary_file, "-ansi", "-pedantic", "-fno-asm"], stdin = None, stdout = None, stderr = ferr)
    ...
    if ret_val == 0:
        try:
            exe_val = subprocess.run([binary_file], stdin = fin, stdout = fout, stderr = ferr, timeout=2, check=True, shell=False).returncode
